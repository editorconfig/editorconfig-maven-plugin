...
public class PluginCharsetDetector {
  ...
  public synchronized List<Charset> detect(byte[] bytes) {
    ...
    if (universalDetector.isDone()) {
      // "done" means that the detector is strongly convinced that it detected the charset right
      charsets = extractSingleDetectedCharset();
    } else {
      // we can still have detected charset out of what we have even if the detector is not done
      ...
      if (detectedCharset != null && !detectedCharset.isEmpty()) {
        ...
        if (StandardCharsets.US_ASCII.equals(javaCharset)) {
          ...
          return Charset.from(javaCharset).map(Collections::singletonList).orElseGet(List::of);
        }
      }

      // if the detector is not sure at all, we still fallback to current state of input characters as a final option
  ...
  private List<Charset> extractSingleDetectedCharset() {
    return Optional
        .ofNullable(universalDetector.getDetectedCharset())
